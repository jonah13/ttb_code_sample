...
//echo '<pre>'.print_r($hierarchy, true).'</pre>';
	...
	function display_inputs_tree($tree, $indent = 0, $group_data)
	{
		$margin = $indent*30;
		$id = '';
		if($indent == 0)
		...
		foreach($tree as $elmt)
		{
			...
			{
				echo '<li><input type="checkbox" class="checkbox subgroup_checkbox" name="subgroups[]" id="subgroup_'.$elmt['ID'].'" value="'.$elmt['ID'].'" '.$t.' '.set_checkbox('subgroups[]', $elmt['ID'], ($group_data['ID'] == $elmt['ID'])?TRUE:FALSE).' /><label for="subgroup_'.$elmt['ID'].'">'.$elmt['name'].'</label>';
				display_inputs_tree($elmt['children'], $indent, $group_data);
			}
			else
				echo '<li><input type="checkbox" class="checkbox location_checkbox" name="locations[]" id="loc_'.$elmt['ID'].'" value="'.$elmt['ID'].'" '.$t.' '.set_checkbox('locations[]', $elmt['ID']).' /><label style="color:#1B0062; font-weight:normal;" for="loc_'.$elmt['ID'].'">'.$elmt['name'].'</label>';
			echo '</li>';
...
?>		
	...
	<div id="content_wrapper">
		...
		<div class="box">
			...
			<form id="add_group_form" class="standard indent_0" method="post" action="<?php echo site_url('admin_companies/submit_edit_group/'.$group_data['ID']); ?>" >
				...
				<p>
					...
					<?php 
						if(empty($group_types))
							echo '<input type="text" name="type" id="type" value="'.set_value('type', $group_data['type']).'" /> '.form_error('type').'<br />';
						else
						{
							echo '<div>';
							...
							foreach($group_types as $type)
								echo '	<option value="'.$type.'" '.set_select('type', $type, ($group_data['type'] == $type)?TRUE:FALSE).'>'.$type.'</option>';
							echo '	</select>';
			...
			<div  class="primary_form" style="padding-bottom:46px;">
				<h3>Locations/sub groups already in the Group</h3> <?php echo form_error('locations[]'); ?> <?php echo form_error('subgroups[]'); ?>
				<p>Uncheck locations or sub groups to remove them from this group</p>
				<p>
				<?php 
					display_inputs_tree($hierarchy, 0, $group_data);
				?>
				</p>
				<h3>Sub groups that you can add to this group:</h3>
				<?php 
					if(!empty($other_groups))
					{
						echo '<ul class="form_elements">';
						foreach($other_groups as $elmt)
							echo '<li><input type="checkbox" class="checkbox subgroup_checkbox" name="subgroups[]" id="subgroup_'.$elmt['ID'].'" value="'.$elmt['ID'].'" '.set_checkbox('subgroups[]', $elmt['ID']).' /><label for="subgroup_'.$elmt['ID'].'">'.$elmt['name'].'</label></li>';
						echo '</ul>';
					}
					else
						echo '<p>N/A</p>';
				?>
				<h3>Locations that you can add to this group:</h3>
				<?php 
					if(!empty($other_locations))
					{
						echo '<ul class="form_elements">';
						foreach($other_locations as $elmt)
							echo '<li><input type="checkbox" class="checkbox location_checkbox" name="locations[]" id="loc_'.$elmt['ID'].'" value="'.$elmt['ID'].'" '.set_checkbox('locations[]', $elmt['ID']).' /><label style="color:#1B0062; font-weight:normal;" for="loc_'.$elmt['ID'].'">'.$elmt['name'].'</label></li>';
						echo '</ul>';
					}
					else
						echo '<p>N/A</p>';
