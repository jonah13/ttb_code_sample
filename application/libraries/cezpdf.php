...
function ezProcessText($text){
  ...
  // purposes
  $search = array('<u>','<U>','</u>','</U>','<b>','<B>','</b>','</B>');
	$replace = array('<c:uline>','<c:uline>','</c:uline>','</c:uline>','<c:bold>','<c:bold>','</c:bold>','</c:bold>');
...
function bold($info)
{
	switch($info['status'])
	{
		case 'start':
		case 'sol':
		$this->selectFont('./fonts/Helvetica-Bold.afm');
		// the beginning of the bold zone
		break;
		case 'end':
		case 'eol':
		// the end of the bold zone
		// assume that it is the most recent opening which has closed
